...
void check_idle(void * arg)
{
	if(sh->CurIdleNum > MaxIdleNum) // check MaxIdleNum
	{
		...
		while(sh->CurIdleNum > StartProcess)
		{
	...
	if(sh->CurIdleNum < MinIdleNum && sh->CurTotalNum < MaxChilds) // check MinIdleNum
	{
		while(sh->CurIdleNum < StartProcess)
		{
			pthread_mutex_lock(&counter_mutex);
...
void print_client(void * arg)
{
	...
	for (int i = repeat-1; i >= 0; i--)
	{
		...
		{
			printf("%d	%s	%d	%d	%s", num, sh->history[i].ip_addr, sh->history[i].pid, sh->history[i].port, sh->history[i].time);
			sprintf(tmp_string, "%d	%s	%d	%d	%s\r\n", num, sh->history[i].ip_addr, sh->history[i].pid, sh->history[i].port, sh->history[i].time);
			strcat(log_string, tmp_string);
...
void reduce_idle(void * arg)
{
	...
	for(int i=0; i < sh->CurTotalNum; i++)
	{
		if(child_pids[i] == tmp_pid && tmp_pid != 0) // find dead child in pid array
		{
			...
			printf("[%s] %d process is teminated\r\n", tmp_time, tmp_pid); // terminated message
			sprintf(log_string, "[%s] %d process is teminated\r\n", tmp_time, tmp_pid);
...
void child_main()
{
	...
	struct timeval tp_start, tp_end, tp_result;
	...
	while (1) // always on
	{
		...
		if (strcmp(url, "/") != 0) // not ROOT_PATH,
			strcat(path, url); // path = ROOT_PATH/input
		...
		timersub(&tp_end, &tp_start, &tp_result);
		sleep(5);
		...
		sprintf(tmp_string, "PID : %d\r\n", getpid());
		strcat(log_string, tmp_string);
		sprintf(tmp_string, "CONNECTING TIME : %ld(us)\r\n", (long int)tp_result.tv_usec);
		strcat(log_string, tmp_string);
		sprintf(tmp_string, "====================================================\r\n\r\n");
		strcat(log_string, tmp_string);
		sprintf(tmp_string, "[%s] idleProcessCount : %d\r\n", tmp_time, sh->CurIdleNum);
...
void main()
{
	...
	// set semaphore
	mysem = sem_open("40302", O_CREAT | O_EXCL, 0700, 1);
	sem_close(mysem);

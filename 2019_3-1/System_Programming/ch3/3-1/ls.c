...
// -------------------------------------------------------	//
// Title : System Programming Assignment #3-1 (html_ls)	//
...
void view_advanced_list(char * path, char * filename, struct stat * buf)
{
	...
	if (f_stat->permission[0] == 'l') // symbolic link -> green color
	{
		...
		char link_file[256];
		char ROOT[1024];
		strcpy(ROOT, "/home/sp2015722087");
		
		strcpy(link_file, path);
		
		if (f_stat->linkpath == NULL)
			p_path = "Non path";
		else if ( strncmp( ROOT, f_stat->linkpath, strlen(ROOT) ) == 0 ) // linked file and sybolic link in same directory.
			p_path = &f_stat->linkpath[strlen(ROOT)]; // except path, only file name.
		else // different directory.
			p_path = f_stat->linkpath; // path/filename
		
		strcpy(path, p_path);
		
		if( strncmp( link_file, f_stat->linkpath, strlen(link_file) - strlen(filename) ) == 0 )
			p_path = &f_stat->linkpath[strlen(link_file) - strlen(filename)];
			
		printf("ROOT : %s\nfilename : %s\npath : %s\nlinkpath : %s\np_path : %s\n", ROOT,filename,path, f_stat->linkpath, p_path);
...
void sort_folder(char ** f_li, int cnt)
{
	...
	for (int a = 0; a < cnt-1; a++)
	{
		for (int b = 0; b < cnt - 1; b++)
		{
			...
			if (a == cnt - 2 && flag_final == 1 && flag_l==100) // last cycle, f_li[b] is end of sorting, plus path string
			{// abs path all file
				if (strcmp(origin_wd, final_wd) != 0) // if file and another working directory,
				{
					...
					if (b == cnt - 2)
					{
						strcpy(tmp, f_li[b + 1]); // tmp = f_li
						strcpy(f_li[b + 1], final_wd); // f_li = final_wd
						strcat(f_li[b + 1], "/"); // final_wd /
						strcat(f_li[b + 1], tmp); // final_wd / f_li
					}
				}
			}
...
int final_ls(char * input)
{
	...
	if (flag_p != -1)
	{
		// check need close table ?
		if (flag_table == 0 && strcmp(final_wd, origin_wd) != 0)
		{
			fputs("</table>\r\n", file);
			flag_table = 1; // if close table -> set flag_table.
			fputs("<br><br>\r\n", file);
	...
	if (strlen(input) != 1)
	{
		...
		else if (s_cnt != strlen(input)-2) // '-/A_what*'
		{
			...
			flag_final = 1;
			tmp_type = advanced_ls(real_path);
			flag_final = 0;
